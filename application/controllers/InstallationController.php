...
class InstallationController extends SanSIS_Controller_Action
{
	...
	protected $title 					= 'Assistente de configuração de instalação do aplicativo';
...
// 					var_dump($_SESSION);die;
		...
		{
			$this->view->form->populate($this->getRequest()->getParams());
			$this->addInstantMessage(SanSIS_Message::TYPE_ERROR, SanSIS_Message::MSG_DBINSTALLERROR.'<br/><br/>'.$e->getMessage());
		}
	}
...
// 			echo fileperms($appini_path);die;
		...
		{
			$this->view->form->populate($this->getRequest()->getParams());
			$this->addInstantMessage(SanSIS_Message::TYPE_ERROR, SanSIS_Message::MSG_DBUSERERROR.'<br/><br/>'.$e->getMessage());
		}
	}
		...
		try
		{
			if($this->getRequest()->getPost())
			{
				...
				if ($this->view->form->isValid($this->getRequest()->getPost()))
				{
					...
					$service->saveInstall($this->getRequest()->getParams());
					$this->addMessage(SanSIS_Message::TYPE_SUCCESS, SanSIS_Message::MSG_SUCCESS, $this->successTarget);
				}
				else
				{
