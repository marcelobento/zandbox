...
abstract class SanSIS_Controller_CrudHTMLSimples extends SanSIS_Controller_CrudBase
{
	...
	public function listAction()
	{
		...
		$data = $this->service->getFormData($this->getFormDataOnly($this->view->form));
		...
		{			
			...
			{	
				$this->view->list = $this->service->getList();
				
				if($this->view->form->isValid($this->getRequest()->getPost()))
				{					
					$this->service->save($this->getFormDataOnly($this->view->form));
			...
			else
			{
				...
				$this->view->list = $this->service->getList($this->getFormDataOnly($this->view->form));
	...
	public function populateForm($id, $extras = null)
	{
		...
		$objeto = $this->service->load($id);
		// Popula os campos do formulário
		$map = $this->service->getMapping();
		foreach($map as $name)
		{
